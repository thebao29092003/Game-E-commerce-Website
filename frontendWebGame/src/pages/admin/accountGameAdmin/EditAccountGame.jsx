...
import { useEditAccountGameMutation } from "../../../features/accountGameApi/accountGameApiSlice";
import { showAlert } from "../../../utility/popup/Popup";

const EditAccountGame = () => {
  const location = useLocation()
  const  {accountGameOld, game}  = location.state || {}; 

  const [toggleIcon, setToggleIcon] = useState(false)
  const [showHidePwd, setShowHidePwd] = useState("password")

  const [accountGame, setAccountGame] = useState({
    accountGameId: accountGameOld[0],
    username: accountGameOld[1],
    password: accountGameOld[2]
  })

  const [triggerEditAccountGame] = useEditAccountGameMutation()
  ...
  const handleAddAccount = async () => {
    ...
    try {
      const result = await triggerEditAccountGame(accountGame).unwrap(); // Thêm unwrap() để bắt lỗi
      if (result) {
        ...
        showAlert("Thông báo !", "Sửa tài khoản game thành công", "success")
     
      }
    } catch (error) {
      // Xử lý lỗi từ API
      console.error("Lỗi khi sửa tài khoản game:", error);
    showAlert("Thông báo !", "sửa tài khoản game thất bại", "error")
    }
  }
      ...
      <div className="list-game-page">
        ...
        <div className="list-game-content">
          <div className="header-btn">
            <h2>
              Tài khoản của game: {game ? game[1] : "Không tìm thấy game"}
            </h2>
            <img
...
export default EditAccountGame;
